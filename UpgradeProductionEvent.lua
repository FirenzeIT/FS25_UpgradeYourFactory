...
function UpgradeProductionEvent.new(prodpoint, newLevel)
...
function UpgradeProductionEvent:run(connection)
    ...
    if prodpoint ~= nil then
		...
		if g_currentMission ~= nil and g_currentMission:getIsServer() then
			...
			if self.newLevel > oldLevel then
				price = prodpoint.owningPlaceable.upgradePrice or 0
			elseif self.newLevel < oldLevel then
				local basePrice = prodpoint.owningPlaceable.price or 0
				price = math.floor(basePrice + basePrice * 0.1 * (oldLevel - 1))
			end
